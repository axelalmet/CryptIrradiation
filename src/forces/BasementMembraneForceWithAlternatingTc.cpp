#include "BasementMembraneForceWithAlternatingTc.hpp"
 ...
 */
BasementMembraneForceWithAlternatingTc::BasementMembraneForceWithAlternatingTc()
:  AbstractForce<2>(),
   mBasementMembraneParameter(DOUBLE_UNSET),
   mTargetCurvature(DOUBLE_UNSET),
   mNumberAlternatingRegions(1)
   {
   }
...
void BasementMembraneForceWithAlternatingTc::SetBasementMembraneParameter(double basementMembraneParameter)
{
	mBasementMembraneParameter = basementMembraneParameter;
}

double BasementMembraneForceWithAlternatingTc::GetBasementMembraneParameter()
{
	return mBasementMembraneParameter;
}


void BasementMembraneForceWithAlternatingTc::SetTargetCurvature(double targetCurvature)
{ 
	mTargetCurvature = targetCurvature;
}


double BasementMembraneForceWithAlternatingTc::GetTargetCurvature()
{
	return mTargetCurvature;
}

void BasementMembraneForceWithAlternatingTc::SetNumberAlternatingRegions(unsigned numberAlternatingRegions)
{
	mNumberAlternatingRegions = numberAlternatingRegions;
}

unsigned BasementMembraneForceWithAlternatingTc::GetNumberAlternatingRegions()
{
	return mNumberAlternatingRegions;
}

void BasementMembraneForceWithAlternatingTc::RemoveDuplicates1D(std::vector<unsigned>& rVectorWithDuplicates)
{
...
std::vector<c_vector<unsigned, 2> > BasementMembraneForceWithAlternatingTc::GetEpithelialStromalPairs(AbstractCellPopulation<2>& rCellPopulation)
{
			...
			++cell_iter)
	{
		...
		if ( (p_type->IsType<StromalCellProliferativeType>()==false) && (!cell_iter->IsDead()) )	// an epithelial cell
		{
					...
					++iter)
			{
				...
				if (element_contains_ghost_nodes==false)
				{
					...
					for (unsigned local_index=0; local_index<3; local_index++)
					{
						...
						if (p_cell->GetCellProliferativeType()->IsType<StromalCellProliferativeType>()==true)
						{
...
bool BasementMembraneForceWithAlternatingTc::DoesElementContainGhostNodes(AbstractCellPopulation<2>& rCellPopulation, unsigned elementIndex)
{
	MeshBasedCellPopulation<2>* p_tissue = static_cast<MeshBasedCellPopulation<2>*>(&rCellPopulation);
...
c_vector<double,2> BasementMembraneForceWithAlternatingTc::GetCryptWidthExtremes(AbstractCellPopulation<2>& rCellPopulation)
{
	...
	double current_width_coordinates;
			...
			++cell_iter)
	{
		...
		// Need these to not be stromal cells
		if (cell_iter->GetCellProliferativeType()->IsType<StromalCellProliferativeType>()==false)
		{
			unsigned node_index = rCellPopulation.GetLocationIndexUsingCell(*cell_iter);

			current_width_coordinates = rCellPopulation.GetNode(node_index)->rGetLocation()[0];

			if (current_width_coordinates > max_width)
			{
				max_width = current_width_coordinates;
			}
			else if (current_width_coordinates < min_width)
			{
				min_width = current_width_coordinates;
			}
		}
	}
...
double BasementMembraneForceWithAlternatingTc::GetCurvatureFromNodePair(AbstractCellPopulation<2>& rCellPopulation, unsigned epithelialNodeIndex,
		unsigned stromalNodeIndex)
{
	...
	spring_midpoint_b = rCellPopulation.GetNode(epithelialNodeIndex)->rGetLocation() + 0.5*(rCellPopulation.rGetMesh().GetVectorFromAtoB(rCellPopulation.GetNode(epithelialNodeIndex)->rGetLocation(), rCellPopulation.GetNode(stromalNodeIndex)->rGetLocation()));
	...
	else
	{
				...
				++elem_iter)
		{
			...
			boost::shared_ptr<AbstractCellProperty> p_E_type = rCellPopulation.GetCellUsingLocationIndex(E)->GetCellProliferativeType();
			assert(!p_E_type->IsType<StromalCellProliferativeType>());
			...
			boost::shared_ptr<AbstractCellProperty> p_S_type = rCellPopulation.GetCellUsingLocationIndex(S)->GetCellProliferativeType();
			assert(p_S_type->IsType<StromalCellProliferativeType>());
			 ...
			 */
			c_vector<double, 2> vector_S_to_E = p_tissue->rGetMesh().GetVectorFromAtoB(rCellPopulation.GetNode(S)->rGetLocation(),rCellPopulation.GetNode(E)->rGetLocation());
			c_vector<double, 2> vector_E_to_S = p_tissue->rGetMesh().GetVectorFromAtoB(rCellPopulation.GetNode(E)->rGetLocation(),rCellPopulation.GetNode(S)->rGetLocation());
			c_vector<double, 2> vector_E_to_P = p_tissue->rGetMesh().GetVectorFromAtoB(rCellPopulation.GetNode(E)->rGetLocation(),rCellPopulation.GetNode(P)->rGetLocation());
			c_vector<double, 2> vector_S_to_P = vector_S_to_E + vector_E_to_P;
			c_vector<double, 2> vector_P_to_S = p_tissue->rGetMesh().GetVectorFromAtoB(rCellPopulation.GetNode(P)->rGetLocation(),rCellPopulation.GetNode(S)->rGetLocation());
			...
			CellPtr p_cell_P = rCellPopulation.GetCellUsingLocationIndex(P);

			if ( (det < 0) && (p_type->IsType<StromalCellProliferativeType>()==false) )	// P = Epithelial, not labelled
			{
				spring_midpoint_c = rCellPopulation.GetNode(E)->rGetLocation() + vector_E_to_P + 0.5*vector_P_to_S;
			}
			else if ((det < 0) && (p_type->IsType<StromalCellProliferativeType>()==true))	// P = stromal
			{
				spring_midpoint_c = rCellPopulation.GetNode(E)->rGetLocation() + 0.5*vector_E_to_P;
			}

			else if ( (det > 0) && (p_type->IsType<StromalCellProliferativeType>()==false) )	// P = Epithelial, not labelled
			{
				spring_midpoint_a = rCellPopulation.GetNode(E)->rGetLocation() + vector_E_to_S + 0.5*vector_S_to_P;
			}

			else if ((det > 0) && (p_type->IsType<StromalCellProliferativeType>()==true))	// P = stromal
			{
				spring_midpoint_a = rCellPopulation.GetNode(E)->rGetLocation() + 0.5*vector_E_to_P;
			}
		...
		c_vector<double, 2> epithelial_location = rCellPopulation.GetNode(epithelialNodeIndex)->rGetLocation();
        double x = epithelial_location[0];

		double target_curvature = GetTargetCurvature();

        double num_alternating_regions = (double)GetNumberAlternatingRegions();
        ...
        // Adjust the target curvature to be negative if we're in the 'protruding' region
        if (cos(2.0 * num_alternating_regions * M_PI * x / (max_width - min_width)) > 0.0)
        {
...
double BasementMembraneForceWithAlternatingTc::GetCurvatureFromMidpoints(AbstractCellPopulation<2>& rCellPopulation,
...
double BasementMembraneForceWithAlternatingTc::FindParametricCurvature(AbstractCellPopulation<2>& rCellPopulation,
		c_vector<double, 2> leftMidpoint,
		c_vector<double, 2> centreMidpoint,
		c_vector<double, 2> rightMidpoint)
{
	...
	c_vector<double, 2> left_to_centre = centreMidpoint - leftMidpoint;
	c_vector<double, 2> centre_to_right = rightMidpoint - centreMidpoint;
	c_vector<double, 2> left_to_right = rightMidpoint - leftMidpoint;

	// Find the parametric intervals
...
unsigned BasementMembraneForceWithAlternatingTc::GetNumContainingElementsWithoutGhostNodes(AbstractCellPopulation<2>& rCellPopulation, unsigned nodeIndex)
{
	MeshBasedCellPopulation<2>* p_tissue = static_cast<MeshBasedCellPopulation<2>*>(&rCellPopulation);
...
std::set<unsigned> BasementMembraneForceWithAlternatingTc::GetNeighbouringNodeIndices(AbstractCellPopulation<2>& rCellPopulation, unsigned nodeIndex)
{
	MeshBasedCellPopulation<2>* p_tissue = static_cast<MeshBasedCellPopulation<2>*>(&rCellPopulation);
...
bool BasementMembraneForceWithAlternatingTc::HasEpithelialCellDetachedFromBasementMembrane(AbstractCellPopulation<2>& rCellPopulation, unsigned nodeIndex)
{
	MeshBasedCellPopulation<2>* p_tissue = static_cast<MeshBasedCellPopulation<2>*>(&rCellPopulation);
			...
			++neighbour_iter)
	{
		...
		if ( (!p_tissue->IsGhostNode(*neighbour_iter)) && (p_type->IsType<StromalCellProliferativeType>()==true))
		{
...
void BasementMembraneForceWithAlternatingTc::AddForceContribution(AbstractCellPopulation<2>& rCellPopulation)
{
	MeshBasedCellPopulation<2>* p_tissue = static_cast<MeshBasedCellPopulation<2>*>(&rCellPopulation);
	...
	for (unsigned i=0; i<node_pairs.size(); i++)
	{
		...
		assert(p_cell_epithelial->GetCellProliferativeType()->IsType<StromalCellProliferativeType>() == false);

		CellPtr p_cell_stromal = p_tissue->GetCellUsingLocationIndex(stromal_node_index);
		assert(p_cell_stromal->GetCellProliferativeType()->IsType<StromalCellProliferativeType>() == true);
...
void BasementMembraneForceWithAlternatingTc::OutputForceParameters(out_stream& rParamsFile)
{
	...
	*rParamsFile <<  "\t\t\t<NumberAlternatingRegions>" << mNumberAlternatingRegions << "</NumberAlternatingRegions> \n";
...
CHASTE_CLASS_EXPORT(BasementMembraneForceWithAlternatingTc)
